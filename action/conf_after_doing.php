...
if (isset($_GET['cancel'])) {
    $detail = $_GET['cancel'];
    echo 'Welcome Zemist TK';
    $sql = "SELECT * FROM repair_status WHERE get_r_id = '$get_r_id' AND del_flg = '0' AND status_id = '$status_id' ORDER BY rs_date_time DESC LIMIT 1";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result)) {
        $row = mysqli_fetch_array($result);
        if ($row['rs_id'] > 0) {

            $sql_c = "SELECT * FROM repair_status WHERE get_r_id = '$get_r_id' AND del_flg = '0' AND status_id = '$status_id' ORDER BY rs_id DESC";
            $result_c = mysqli_query($conn, $sql_c);

            if (mysqli_num_rows($result)) {
                $rs_id = $row['rs_id'];
                $text = $detail;
                $translatedText = str_replace(array("%20", "'"), array(" ", "'"), $text);
                $sql = "UPDATE repair_status SET rs_conf = '0' , rs_conf_date = NOW() ,rs_cancel_detail = '$translatedText' WHERE rs_id = '$rs_id'";
                ...
                if ($result1) {
                    ...
                    if ($result_update) {
                        $sql_update = "UPDATE parts_use SET del_flg = '1' WHERE rs_id = '$rs_id'"; //แค่ครั้งเดียว
                        $result_update1 = mysqli_query($conn, $sql_update);
                        if ($result_update1) {
