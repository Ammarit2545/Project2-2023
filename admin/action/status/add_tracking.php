...
while (isset($_POST['com_y_' . $i])) {
    $com_y = $_POST['com_y_' . $i];
    $tracking_number = $_POST['tracking_number' . $i];
    $get_d_id = $_POST['get_d_id_' . $i];

    $sql = "SELECT * FROM tracking WHERE t_parcel = '$tracking_number' AND del_flg = 0";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_array($result);

    if ($row == NULL) {
        $sql = "INSERT INTO tracking(t_parcel, t_c_id, t_date_in) VALUES ('$tracking_number', '$com_y', NOW())";
        $result = mysqli_query($conn, $sql);
        ...
        $sql_update_detail = "UPDATE get_detail SET t_id = '$t_id' WHERE get_d_id = '$get_d_id'";
        $result_update_detail = mysqli_query($conn, $sql_update_detail);
