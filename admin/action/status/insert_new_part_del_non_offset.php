...
if ($status_id == 17) {
    ...
    $sql_c = "SELECT rs_id FROM repair_detail 
        ...
        WHERE get_detail.get_r_id = '$get_r_id' AND repair_detail.del_flg = '0' AND get_repair.del_flg = '0'ORDER BY rs_id DESC LIMIT 1;";
    ...
    if (mysqli_num_rows($result_c)) {
        ...
        $sql_u_rd = "UPDATE `repair_detail` SET `del_flg` = '1' WHERE `rs_id` = '$rs_id'";
        $result_u_rd = mysqli_query($conn, $sql_u_rd);

        if ($result_u_rd) {
            // Check if parts_use entry already exists for this repair
            $sql_u_rd = "UPDATE `parts_use` SET `del_flg` = '1' WHERE `rs_id` = '$rs_id'";
            $result_u_rd = mysqli_query($conn, $sql_u_rd);
        }
    }
}
