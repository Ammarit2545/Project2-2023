...
<body>
        ...
        <table class="table table-striped table-hover"> <!-- Add table-bordered class -->
            <thead>
                <tr>
                    ...
                    <th scope="col" width="200px">ชื่อ-นามสกุล</th>
            ...
            <tbody>
                ...
                $i = 0;
                $p_stock = 0;
                $sql = "SELECT * FROM employee LEFT JOIN role ON employee.role_id = role.role_id WHERE employee.del_flg = '0'";
                ...
                ?>
                    <tr>
                        ...
                        <td>
                            <?php echo $row['e_id'] ?? "ไม่มีข้อมูล"; ?>
                        </td>
                        <td>
                            <center>
                                <?php echo $row['e_fname'] . " " . $row['e_lname'] ?? "ไม่มีข้อมูล"; ?>
                            </center>
                        </td>
                        <td>
                            <?php echo $row['e_email'] ?? "ไม่มีข้อมูล"; ?>
                        </td>
                        <td>
                            <?php echo $row['e_tel'] ?? "ไม่มีข้อมูล"; ?>
                        </td>
                        <td>
                            <?php echo $row['role_name'] ?? "ไม่มีข้อมูล"; ?>
                        </td>
                        <td>
                            <?php echo $row['e_salary'] ?? "ไม่มีข้อมูล"; ?>
                        </td>
                    </tr>
                <?php
                }
                ?>
                <!-- <tr>
                    <td colspan="5">ยอดอะไหล่ทั้งหมดรวมทั้งสิ้น <?= $i ?> รายการ </td>
                    <td>คงเหลือ</td>
    ...
    $mpdf->WriteHTML($html);
    $filename = date('Y-m-d') . ' - รายงานยอดบุคลากร_Report.pdf';
