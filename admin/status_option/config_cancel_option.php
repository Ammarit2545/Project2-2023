<center>
    ...
    <?php
    $latestRepairStatusQuery = "SELECT status_id, rs_conf FROM repair_status WHERE del_flg = 0 AND get_r_id = '$get_r_id' ORDER BY rs_id DESC LIMIT 1";
    $latestRepairStatusResult = mysqli_query($conn, $latestRepairStatusQuery);
    $latestRepairStatusRow = mysqli_fetch_array($latestRepairStatusResult);

    if ($latestRepairStatusRow['rs_conf'] == 0 && $latestRepairStatusRow['status_id'] == 13) {
        $statusTypeQuery = "SELECT * FROM status_type WHERE status_id = '17'";
        $statusTypeResult = mysqli_query($conn, $statusTypeQuery);
        $statusTypeRow = mysqli_fetch_array($statusTypeResult);
    ?>
        <button class="btn btn-success" style="background-color:<?= $statusTypeRow['status_color'] ?>; border : <?= $statusTypeRow['status_color'] ?>" onclick="showOfferStatus('<?php echo $statusTypeRow['id']; ?>')">
            <?= $statusTypeRow['status_name'] ?>
        </button>
    <?php
    }
...
<div id="cancel_value_code" style="display: none;">
    ...
    <form id="cancel_status" action="action/status/add_conf_offer.php" method="POST" enctype="multipart/form-data">
        ...
        <textarea class="form-control auto-expand" name="rs_detail" id="cancelFormControlTextareaConf" rows="3" required placeholder="กรอกรายละเอียดในการยกเลิกคำส่งซ่อม">ยกเลิกการซ่อม</textarea>
        <input type="text" name="get_r_id" value="<?= $get_r_id ?>" hidden>
...
<div id="cancel_value_code" style="display: none;">
    ...
    <form id="confirm_cen_config" action="action/status/doing_status.php" method="POST" enctype="multipart/form-data">
        <label for="cancelFormControlTextareaConf" class="form-label">กรุณาใส่รายละเอียดเพื่อทำการ <p style="display:inline; color : <?= $row_q['status_color'] ?>"> <?= $row_q['status_name'] ?></p> :</label>
        <textarea class="form-control auto-expand" name="rs_detail" id="cancelFormControlTextareaConf" rows="3" required placeholder="กรอกรายละเอียดในการยกเลิกคำส่งซ่อม">ทางเรากำลังดำเนินการส่งเครื่องเสียงให้ท่านผ่านผู้ให้บริการจัดส่งพัสดุ</textarea>
        <input type="text" name="get_r_id" value="<?= $get_r_id ?>" hidden>
        <input type="text" name="status_id" value="<?= $row_q['status_id'] ?>" hidden>
        <br>
        <p style="color:red">*** โปรดกรอกรายละเอียดข้างต้นก่อนทำการเพิ่มรูปภาพ ***</p>
...
<div id="status_doing" style="display: none;">
    ...
    <form id="cancel_status_id_conf_get" action="action/status/add_conf_cancel.php" method="POST" enctype="multipart/form-data">
        ...
        <textarea class="form-control auto-expand" name="rs_detail" id="cancelFormControlTextareaConf" rows="3" required placeholder="กรอกรายละเอียดในการยกเลิกคำส่งซ่อม">ทางเราจำเป็นต้องใช้อะไหล่ดังนี้   </textarea>
        <input type="text" name="get_r_id" value="<?= $get_r_id ?>" hidden>
...
<div id="17_offer" style="display: none;">
    <hr>
    <br>
    <h1 class="m-0 font-weight-bold text-primary">ตอบกลับ </h1>
    <br>
...
<script>
    function showCancelValue() {
        ...
        document.getElementById('17_offer').style.display = 'none';
    ...
    function showOfferStatus() {
        ...
        document.getElementById('status_doing').style.display = 'none';
        document.getElementById('doing_status').style.display = 'none';
        document.getElementById('17_offer').style.display = 'block';
    }
